...
const KPIExitComponentModal: React.FC<KPIExitComponentModalProps> = ({
  ...
  const fetchExitComponentData = async () => {
    ...
    setError(null);
    try {
      console.log('Fetching exit component data for KPI:', kpiId);
      const response = await apiClient.get(`/kpis/${kpiId}/exit-components`);
      
      console.log('Exit component API response:', response.data);
      ...
      if (response.data) {
        ...
        console.log('Exit components found:', data.exitComponents);
        console.log('Existing objectives found:', data.existingObjectives);
        console.log('KPI info:', data.kpi);
        ...
        <div className="inline-block w-full max-w-2xl px-6 py-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl sm:max-w-2xl">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-lg font-medium leading-6 text-gray-900">
                {exitComponents.length > 0 ? `Manage ${exitComponents[0].componentName}s` : 'Manage Exit Components'}
              </h3>
              {kpiInfo && (
          ...
          {!loading && (
            <div className="space-y-6">
              {(() => {
                console.log('Rendering content with exitComponents:', exitComponents, 'length:', exitComponents.length);
                return null;
              })()}
              ...
              {exitComponents.length > 0 && (
                <div>
                  <h4 className="text-md font-medium text-gray-900 mb-3">
                    Available {exitComponents[0].componentName} Templates for this Level
                  </h4>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          >
                            ...
                            Add {getExitComponentDisplayName(template)}
                          </button>
                        </div>
                      </div>
                    ))}
              ...
              {existingObjectives.length > 0 && (
                <div>
                  <h4 className="text-md font-medium text-gray-900 mb-3">
                    Existing {exitComponents.length > 0 ? exitComponents[0].componentName + 's' : 'Exit Components'}
                  </h4>
                  <div className="space-y-2">
              ...
              {showCreateForm && (
                <div className="border-t pt-4">
                  <h4 className="text-md font-medium text-gray-900 mb-3">
                    Create New {exitComponents.find(t => t.id === form.parentExitComponentId)?.componentName || 'Exit Component'}
                  </h4>
                  <form onSubmit={handleCreateExitComponent} className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        {exitComponents.find(t => t.id === form.parentExitComponentId)?.componentName || 'Exit Component'} Title *
                      </label>
                      <input
                        type="text"
                        value={form.name}
                        ...
                        placeholder={`Enter ${exitComponents.find(t => t.id === form.parentExitComponentId)?.componentName.toLowerCase() || 'exit component'} title`}
                        required
                      />
                    </div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        {exitComponents.find(t => t.id === form.parentExitComponentId)?.componentName || 'Exit Component'} Description
                      </label>
                      <textarea
                        ...
                        rows={3}
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                        placeholder={`Enter ${exitComponents.find(t => t.id === form.parentExitComponentId)?.componentName.toLowerCase() || 'exit component'} description (optional)`}
                      />
                    </div>
                    ...
                    <div className="flex justify-end space-x-3">
                      ...
                      >
                        {submitting ? 'Creating...' : `Create ${exitComponents.find(t => t.id === form.parentExitComponentId)?.componentName || 'Exit Component'}`}
                      </button>
                    </div>
                  </form>
                </div>
